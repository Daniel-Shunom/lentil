...
pub fn create_bin_handler(state: wt.BinState) {
  ...
  let assert Ok(_)  = supervisor.start_spec(supervisor.Spec(
    Nil, 10, 5, supervisor.add(_, worker)
  ))
...
fn bin_handler(msg: wt.Retry, state: wt.BinState) -> actor.Next(wt.Retry, wt.BinState) {
